...
export interface CubesRef {
  triggerRipple: (rowHit?: number, colHit?: number) => void;
}
...
const Cubes = forwardRef<CubesRef, CubesProps>(({
  ...
  const triggerRipple = useCallback(
    (rowHit?: number, colHit?: number) => {
      ...
      // Default to center of grid if not provided
      const hitRow = rowHit ?? Math.floor(gridSize / 2);
      const hitCol = colHit ?? Math.floor(gridSize / 2);
      ...
      sceneRef.current
        ...
        .forEach((cube) => {
          const r = +cube.dataset.row!;
          const c = +cube.dataset.col!;
          const dist = Math.hypot(r - hitRow, c - hitCol);
  ...
  const onClick = useCallback(
    ...
    [rippleOnClick, gridSize, triggerRipple]
  );
  ...
  useEffect(() => {
    ...
    const speed = 0.02;
    const loop = () => {
      // Only auto-animate when user is not hovering and not actively interacting
      // When not hovering, reset all cubes to flat position instead of tilting
      if (!userActiveRef.current && !isHoveringRef.current) {
        // Reset all cubes to flat position when not hovering
        if (sceneRef.current) {
          sceneRef.current
            .querySelectorAll<HTMLDivElement>(".cube")
            .forEach((cube) => {
  ...
  }, [autoAnimate, gridSize, leaveDur]);

  // Expose triggerRipple via ref
  useImperativeHandle(ref, () => ({
    triggerRipple: (rowHit?: number, colHit?: number) => {
      triggerRipple(rowHit, colHit);
    },
  }), [triggerRipple]);
