...
`;
    ...
    class Advection extends ShaderPass {
      ...
      update(...args: any[]) {
        const { dt: dtProp, isBounce: isBounceProp, BFECC: BFECCProp } =
          (args[0] || {}) as { dt?: number; isBounce?: boolean; BFECC?: boolean };
        if (!this.uniforms) return;
        if (typeof dtProp === "number") this.uniforms.dt.value = dtProp;
        if (typeof isBounceProp === "boolean") this.line.visible = isBounceProp;
        if (typeof BFECCProp === "boolean") this.uniforms.isBFECC.value = BFECCProp;
        super.update();
      }
    }
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(...args: any[]) {
        ...
        const cursorSizeProp = props.cursor_size || 0;
        const cursorSizeX = cursorSizeProp * cellScale.x;
        const cursorSizeY = cursorSizeProp * cellScale.y;
        ...
        uniforms.scale.value.set(cursorSizeProp, cursorSizeProp);
        super.update();
      }
    }
    ...
    class Viscous extends ShaderPass {
      ...
      update(...args: any[]) {
        const { viscous: viscousProp, iterations, dt: dtProp } =
          (args[0] || {}) as { viscous?: number; iterations?: number; dt?: number };
        if (!this.uniforms) return;
        let fbo_in: any;
        let fbo_out: any;
        if (typeof viscousProp === "number") this.uniforms.v.value = viscousProp;
